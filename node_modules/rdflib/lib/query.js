  ...
  */
  ...
  function unifyTerm(self, other, bindings, formula) {
    ...
    if (!actual.complexType) {
      ...
      if (actual.sameTerm(other) || actual.uri && actual.uri === _store.defaultGraphURI) {
  ...
  */
  ...
  function prepare(f, item, bindings) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      if (t.uri && t.uri === _store.defaultGraphURI) {// chrome:session
  ...
  var match = function match(f, g, bindingsSoFar, level, fetcher, localCallback, branch) {
    ...
    if (sf) {
      ...
      for (i = 0; i < n; i++) {
        ...
        if (bindingsSoFar[item.subject] !== undefined && bindingsSoFar[item.subject].uri && sf && sf.getState((0, _uri.docpart)(bindingsSoFar[item.subject].uri)) === 'unrequested') {
        ...
        if (bindingsSoFar[item.object] !== undefined && bindingsSoFar[item.object].uri && sf && sf.getState((0, _uri.docpart)(bindingsSoFar[item.object].uri)) === 'unrequested') {
