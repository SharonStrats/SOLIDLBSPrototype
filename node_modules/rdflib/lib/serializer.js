...
var _xsd = _interopRequireDefault(require("./xsd"));
  ...
  __Serializer.prototype.statementsToNTriples = function (sts) {
    ...
    var termToNT = function termToNT(x) {
      ...
      for (var i = list.length - 1; i >= 0; i--) {
        var bnode = new _blankNode.default();
  ...
  __Serializer.prototype.atomicTermToN3 = function atomicTermToN3(expr, stats) {
    switch (expr.termType) {
      ...
      case 'Literal':
        ...
        if (expr.language) {
          str += '@' + expr.language;
        } else if (!expr.datatype.equals(_xsd.default.string)) {
      ...
      for (var i = 0; i < sts.length; i++) {
        ...
        if (pred.uri.substr(0, liPrefix.length) === liPrefix) {
          ...
          if (number === intNumber.toString()) {
            ...
            pred = new _namedNode.default('http://www.w3.org/1999/02/22-rdf-syntax-ns#li');
          }
        }
        ...
        switch (st.object.termType) {
          ...
          case 'Literal':
            results = results.concat(['<' + t + (st.object.datatype.equals(_xsd.default.string) ? '' : ' rdf:datatype="' + escapeForXML(st.object.datatype.uri) + '"') + (st.object.language ? ' xml:lang="' + st.object.language + '"' : '') + '>' + escapeForXML(st.object.value) + '</' + t + '>']);
            break;
    ...
    function propertyXMLTreeMethod(subject, stats) {
      ...
      for (var i = 0; i < sts.length; i++) {
        ...
        switch (st.object.termType) {
          ...
          case 'Literal':
            results = results.concat(['<' + qname(st.predicate) + (st.object.datatype.equals(_xsd.default.string) ? '' : ' rdf:datatype="' + escapeForXML(st.object.datatype.value) + '"') + (st.object.language ? ' xml:lang="' + st.object.language + '"' : '') + '>' + escapeForXML(st.object.value) + '</' + qname(st.predicate) + '>']);
            break;
